    ...
    <nav className="sticky top-0 left-0 w-full z-50 bg-brand-card border-b border-brand-border py-8 px-6 flex items-center justify-between select-none">
      ...
      <div className="flex items-center gap-4">
        ...
        >
          ...
          >
            <defs>
              <linearGradient id="mt-navbar-logo-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <path
              ...
              stroke="url(#mt-navbar-logo-gradient)"
              strokeWidth="5"
              strokeLinejoin="round"
            />
            <path
              d="M50 28L72 65L50 54L28 65L50 28Z"
              fill="url(#mt-navbar-logo-gradient)"
            />
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="relative">
          {(() => {
            ...
            const userFullName = currentUser.fullName || currentUser.name || 'Dr. Kumar';
            const userRole = currentUser.role || 'Mentor';
            const userEmail = currentUser.email || 'kumar@pp.edu';
            return (
              <>
                <button
                  type="button"
                ...
                >
                  <div className="w-10 h-10 rounded-full border border-cyan-500/35 bg-gradient-to-tr from-cyan-500/15 to-blue-500/10 text-cyan-500 flex items-center justify-center font-extrabold text-sm transition-all duration-300 group-hover:border-cyan-500/60 group-hover:shadow-glow-cyan">
                    {userFullName.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
                  </div>
                  
                  <div className="hidden sm:block text-left">
                    <h4 className="text-xs font-bold text-brand-text leading-tight group-hover:text-cyan-500 transition-colors duration-300">
                      {userFullName}
                    </h4>
