...
import { FiBell, FiMenu, FiUser, FiChevronDown, FiShield } from 'react-icons/fi';
...
const StudentNavbar = ({ 
  ...
  return (
    <nav className="sticky top-0 left-0 w-full z-50 bg-brand-card border-b border-brand-border py-8 px-6 flex items-center justify-between select-none">
      ...
      {/* LEFT: hamburger trigger + brand logo */}
      <div className="flex items-center gap-4">
        <button
          onClick={toggleSidebar}
          className="p-2 rounded-lg border border-brand-border text-brand-text-muted hover:text-brand-text hover:bg-slate-200/50 dark:hover:bg-slate-800/50 transition-colors duration-300 focus:outline-none"
        >
        ...
        >
          ...
          >
            <defs>
              <linearGradient id="st-navbar-logo-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <path
              ...
              stroke="url(#st-navbar-logo-gradient)"
              strokeWidth="5"
              strokeLinejoin="round"
            />
            <path
              d="M50 28L72 65L50 54L28 65L50 28Z"
              fill="url(#st-navbar-logo-gradient)"
            />
      ...
      {/* RIGHT: notifications bell + student avatar */}
      <div className="flex items-center gap-5">
        ...
        <div className="relative">
          {(() => {
            ...
            const userFullName = currentUser.fullName || currentUser.name || 'Student';
            const userRole = currentUser.role || 'Active Member';
            const userEmail = currentUser.email || 'student@pp.edu';
            return (
              <>
                <button
                  type="button"
                ...
                >
                  <div className="w-10 h-10 rounded-full border border-secondary/35 bg-gradient-to-tr from-secondary/15 to-primary/10 text-secondary flex items-center justify-center font-extrabold text-sm transition-all duration-300 group-hover:border-secondary/60 group-hover:shadow-glow-secondary">
                    {userFullName.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
                  </div>
                  
                  <div className="hidden sm:block">
                    <h4 className="text-xs font-bold text-brand-text leading-tight group-hover:text-secondary transition-colors duration-300">
                      {userFullName}
                    </h4>
